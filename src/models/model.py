...
class SMC(nn.Module):
    ...
    def __init__(
        ...
        # decision layers
        self.dc_conv1 = nn.Conv1d(layers[1] * block2.expansion, num_classes, kernel_size=1)
...
class dla(nn.Module):
    ...
    def __init__(self, classes, factor=3):
        ...
        self.bn1 = nn.BatchNorm1d(classes, track_running_stats=False)
        self.bn2 = nn.BatchNorm1d(classes, track_running_stats=False)
...
class DLA(nn.Module):
    ...
    def __init__(self,
        ...
        self.block1 = block(classes=classes, factor=3.0)
        self.block2 = block(classes=classes, factor=4.0)
...
def _emg_model(
    ...
    model = SMC(block1, block2, block3, layers=layers)
    if pretrained:
        pass
    return model


def emgmodel18(pretrained: bool = False, **kwargs: Any) -> SMC:
    return _emg_model(GLInceptionBlock, DWResBlock, DWCBAM, layers=[33, 44])
...
if __name__ == '__main__':
    ...
    # predict_label, extracted_feature = model(test)
